...
class CustomEndpointEnableCommand extends \Symfony\Component\Console\Command\Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        return $this->state->emulateAreaCode(Area::AREA_ADMINHTML, function () use ($input, $output) {
            ...
            if ($targetEndpoint) {
                $config->setDataByPath('ktpl_s3/custom_endpoint/endpoint', $targetEndpoint);
                $config->save();
            }
            if ($targetRegion) {
                $config->setDataByPath('ktpl_s3/custom_endpoint/region', $targetRegion);
                $config->save();
            }
            $config->setDataByPath('ktpl_s3/custom_endpoint/enabled', 1);
            $config->save();
    ...
    protected function getTargetConfigSetting($name, Config $config, InputInterface $input)
    {
        ...
        if ($config->getConfigDataValue(sprintf('ktpl_s3/custom_endpoint/%s', strtolower($name)))) {
            return $config->getConfigDataValue(sprintf('ktpl_s3/custom_endpoint/%s', strtolower($name)));
        }
