    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        return $this->state->emulateAreaCode(Area::AREA_ADMINHTML, function () use ($output) {
            ...
            $output->writeln(sprintf(
                'Access Key ID:     %s',
                $config->getConfigDataValue('ktpl_s3/general/access_key')
            ));
            $output->writeln(sprintf(
                'Secret Access Key: %s',
                $config->getConfigDataValue('ktpl_s3/general/secret_key')
            ));
            $output->writeln(sprintf(
                'Bucket:            %s',
                $config->getConfigDataValue('ktpl_s3/general/bucket')
            ));
            $output->writeln(sprintf(
                'Region:            %s',
                $config->getConfigDataValue('ktpl_s3/general/region')
            ));
            $output->writeln(sprintf(
                'Upload Folder:     %s',
                $config->getConfigDataValue('ktpl_s3/prefix/bucket_folder')
            ));
