    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        return $this->state->emulateAreaCode(Area::AREA_ADMINHTML, function () use ($output) {
            ...
            try {
                ...
                // echo "<pre>";print_r($client->determineBucketRegion($this->helper->getBucket()));die;
            } catch (\Exception $e) {
                $output->writeln(sprintf('<error>%s</error>', $e->getMessage()));

                return 1;
            }
            // echo "<pre>";print_r($client->doesBucketExist($this->helper->getBucket()));die;
            // echo "<pre>";print_r(get_class_methods($client));die;
